          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {packages.map((item) => (
              <div key={item.name} className={`relative flex flex-col rounded-[1.7rem] border p-7 ${item.featured ? "border-[#0f5752] bg-[#f2f8f5] shadow-soft" : "bg-white"}`}>
                {item.featured && <span className="absolute right-6 top-6 rounded-full bg-lime-300 px-3 py-1 text-xs font-semibold text-[#153a35]">Più scelto</span>}
                <p className="text-lg font-semibold">{item.name}</p>
                <div className="mt-6"><span className="text-4xl font-semibold tracking-tight">{item.price}</span><span className="ml-2 text-sm text-slate-500">{item.note}</span></div>
